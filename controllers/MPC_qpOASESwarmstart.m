classdef MPC_qpOASESwarmstart < handle
    ...
    %% methods
    methods
        function self = MPC_qpOASESwarmstart(Mdl, L, R, N, varargin)
...
%                 self.g_pre*[x_k; d_vec_k]; %BEcause I changed how
%                 constraints are handled
            g = self.l_g_pre + ...
                self.g_pre*[x_k; d_vec_k];
            
            % More likely to want options in here
            aux_input = qpOASES_auxInput('x0', self.u_opts);
            [u_vec,~,~,~,lambda,~] = qpOASES(self.H, h, ...
                self.G, self.lb, self.ub, [], g, [], aux_input);
            self.u_opts = u_vec;
...
%             end
        ...
        function initialize_mpc(self, L, R, E_G, J_G, l_g, lb, ub)
